...
const mails = [
    {
        ...
        Body: "mail numero 1",
        RecipientEmail: 'john.alexander@example.com',
    },
    {
        Subject: 'New classmate',
        CategoryFolder: 'Sent',
        SenderEmail: 'michael.foster@example.com',
        Date: '2023-01-23T13:23Z',
        Body: "mail numero 2",
        RecipientEmail: 'michelle.foster@example.com',
    },
    {
        Subject: 'Please confirm your attendance',
        CategoryFolder: 'Sent',
        SenderEmail: 'dries.vincent@example.com',
        Date: '2023-01-23T13:23Z',
        Body: "mail numero 3",
        RecipientEmail: 'michael.foster@example.com',
    },
    ...
    <div class="margins-container flex flex-row">
        <div class="col-start-1 col-span-4 mid-1-2-space">
            <ul role="list" class="divide-y divide-gray-100">
                <li v-for="mail in mails" :key="mail.SenderEmail + mail.Date" class="flex justify-between gap-x-6 py-5"
                    ...
                    <div class="flex gap-x-4">
                        ...
                        <div class="min-w-0 flex-auto">
                            <p class="text-sm font-semibold leading-6 text-gray-900"> {{ mail.Subject }}</p>
                            <p class="text-sm leading-6 text-gray-900"> {{ mail.CategoryFolder }}</p>
                            <p class="mt-1 truncate text-xs leading-5 text-gray-500"> From: {{ mail.SenderEmail }}</p>
                        </div>
                    </div>
                    <div class="hidden sm:flex sm:flex-col sm:items-end">
                        <p class="text-sm leading-6 text-gray-900"> .</p>
                        <p class="text-sm leading-6 text-gray-900"> {{ mail.Date }}</p>
                        <p class="mt-1 text-xs leading-5 text-gray-500"> To: {{ mail.RecipientEmail }}
                        </p>
                    </div>
                </li>
            </ul>
        </div>
        <span class="mail-container-style">
            <p class="bodymail-style"> Here is the body: <br> {{ selectedMail && selectedMail.Body }}</p>
        </span>
    </div>
</template>
